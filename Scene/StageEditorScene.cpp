...
static float DistSqXZ(const Vector3& a, const Vector3& b)
{
...
static int FindNearestGateIndex_Screen(
	const std::vector<GateVisual>& gates,
	POINT mouse,
	const Matrix4x4& viewProj,
	float screenW, float screenH,
	float radiusPx) // 例: 40～80
{
	int best = -1;
	...
	for (int i = 0; i < (int)gates.size(); ++i) {
		Vector2 s{};
		if (!WorldToScreen_RowVector(gates[i].gate.pos, viewProj, screenW, screenH, s)) {
			continue;
		}
		const float dx = s.x - (float)mouse.x;
		const float dy = s.y - (float)mouse.y;
...
static int FindNearestWallIndex_Screen(
	const std::vector<WallSystem::Wall>& walls,
	POINT mouse,
	const Matrix4x4& viewProj,
	float screenW, float screenH,
	float radiusPx)
{
	...
	for (int i = 0; i < (int)walls.size(); ++i) {
		Vector2 s{};
		if (!WorldToScreen_RowVector(walls[i].center, viewProj, screenW, screenH, s)) continue;
...
// =========================
	...
	// ========== クリック選択（P押してない時は選択、P押してる時は配置） ==========
	{
		...
		bool nowL = input.IsMousePressed(0);

		bool clickDown = (nowL && !prevL);
		bool clickRelease = (!nowL && prevL);
		...
		if (!cameraRotating && !placing && clickRelease) {
			POINT mp = downPos; // ★ここが重要：離した位置じゃなく押した位置
			//POINT mp = input.GetMousePos();
			const float W = (float)WinApp::kClientWidth;
			const float H = (float)WinApp::kClientHeight;
			const Matrix4x4& vp = camera_->GetViewProjectionMatrix();
			...
			{
				if (editMode_ == EditMode::Gate) {
					const float pickRadiusPx = 60.0f; // まずは60、まだキツければ80
					const int idx = FindNearestGateIndex_Screen(gates_, mp, vp, W, H, pickRadiusPx);
					if (idx >= 0) selectedGate_ = idx;

				} else if (editMode_ == EditMode::Wall) {
					auto& walls = wallSys_.Walls();
					const float pickRadiusPx = 60.0f;
					const int idx = FindNearestWallIndex_Screen(walls, mp, vp, W, H, pickRadiusPx);
